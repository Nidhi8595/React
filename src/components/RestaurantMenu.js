...
if (!resInfo && !error) return <ShimmerMenu />;

if (error) return <h2 className="text-center mt-10 text-red-600">⚠️ {error}</h2>;
  ...
  const {
    ...
    avgRating,
    totalRatingsString,
  } = infoCard?.card?.card?.info || {}; // Use || {} as a fallback for destructuring

  const deliveryTime = infoCard?.card?.card?.info?.sla?.deliveryTime; // Safely find the card that contains the item cards (menu data)

  // RestaurantMenu.js

  const menuSection = resInfo?.cards?.find(
    (card) => card?.groupedCard?.cardGroupMap?.REGULAR?.cards.length > 0
  ); // RestaurantMenu.js
  // 1. Define the path to the array of menu category cards, providing an empty array as a fallback.
  const regularMenuCards =
    menuSection?.groupedCard?.cardGroupMap?.REGULAR?.cards || [];

  // 2. Now use flatMap safely on the guaranteed array
  const itemCards = regularMenuCards.flatMap(
    (card) => card?.card?.card?.itemCards || []
  );

  console.log(itemCards); // This will now always log an array ([]) if no items are found.

  const itemCardCount = itemCards.length; // ✅ FIX: This is now safe.
    ...
    <div className="menu h-full w-full">
      <header className="menu-header w-full h-64 bg-black text-white flex align-middle justify-center gap-7 py-3.5 shadow-lg shadow-black">
        <div className="menu-header-left">
          {/* <img
            className="topimage w-64 h-56 bg-cover bg-center rounded-lg"
        ...
        <div className="menu-header-right">
          <h1 className="topheading font-medium text-4xl/[36px] pt-2 scale-y-125 ml-2">
            {name}
          </h1>
          <h3 className="topcuisines font-normal pl-4 opacity-70 mt-4">
            {(cuisines || []).join(", ")}
          </h3>
          <div className="bottom flex align-middle gap-5 mt-5 pl-3">
            <div className="avg-rating flex pr-5 border-r-4 border-solid border-slate-500">
              ...
              <h4 className="pt-1.5 text-green-500">
                <AiOutlineStar />
              </h4>
              <h4 className="text-slate-400 pl-1.5 pt-1">
                ({totalRatingsString})
              </h4>
            </div>
      ...
      {itemCardCount > 0 ? (
        <div className="w-5/6 mx-auto flex justify-center align-middle">
          <div className="flex h-20 mt-3 py-5 pl-4 text-3xl text-purple-900 rounded-2xl">
            ...
            <img
              className="pl-4 -mt-1 cover"
              src="https://as2.ftcdn.net/v2/jpg/09/70/93/27/1000_F_970932707_rohMcQ9hgc6RAYjPSZKqUoTM9eUAPTlr.webp"
            />
          </div>
        </div>
      ) : (
      ...
      <div className="menu-main mx-24 mb-12 mt-5">
        ...
        <h3 className="items w-1/2 ml-[218] my-2 text-base opacity-80">
          <b>{itemCardCount}</b> ITEMS
        </h3>
          ...
          {itemCards.length > 0 ? (
            itemCards.map((item) => (
              <div
                ...
                className="menu-card h-48 flex align-middle justify-between gap-4 bg-white p-5 border-b-2 border-b-solid border-b-purple-400"
              >
                <div className="menu-card-left w-2/3 pt-1 pl-2">
                  <div className="flex gap-2 w-full">
                    ...
                    {item.card?.info.isBestseller ? (
                      <div>
                        <img
                          className="h-9 w-9 mb-1  ml-3"
                          src="https://cdn-icons-png.freepik.com/256/8440/8440997.png?ga=GA1.1.1651789925.1730482677&semt=ais_hybrid"
                        />
                      </div>
                    ) : (
                      <div></div>
                  ...
                  <h2 className="menu-name opacity-90 mb-1.5 text-xl font-medium">
                    {item.card?.info.name}
                  </h2>
                  {item.card?.info.ratings?.aggregatedRating.rating ? (
                    <div className="menu-rating mt-1 flex text-lg gap-1 p-0.5 text-purple-600">
                      {item.card?.info.ratings.aggregatedRating.rating}
                      <div className="star mt-[5px]">
                        <AiOutlineStar />
                      </div>
                      <div className="count opacity-90 text-lg pl-1">
                ...
                <div className="menu-card-right flex-col pb-12">
                  ...
                  <div className="text-center -mt-7">
                    <button
                      className="p-1.5 w-28 h-10 rounded-xl bg-white scale-y-105 text-lg text-purple-800 shadow-lg shadow-purple-300 hover:scale-110 hover:text-white hover:bg-purple-900 transition-all duration-[.3s]"
            ...
            <div className="notAvail p-5 text-amber-950 text-2xl align-middle text-center">
      ...
      {isModalOpen && selectedItem && (
        <div className="modal-overlay" onClick={closeModal}>
          <div
            className="detailed-card h-[570px] w-[550px] mt-1 flex flex-col justify-between rounded-2xl bg-white mx-auto"
          ...
          >
            ...
            <div className="px-3 flex justify-between text-purple-900">
              <div className="flex flex-col justify-between">
                ...
                <h2 className="menu-name text-2xl ">
                  {selectedItem.card?.info.name}
                </h2>
                <h3 className="menu-price opacity-80 mt-0.5 font-semibold text-lg ml-1">
                  ₹
                  {selectedItem.card?.info.price / 100 ||
                    selectedItem.card?.info.defaultPrice / 100}
                </h3>
              </div>
              <div className="text-center">
                <button
                  className="mr-1 p-1.5 w-28 h-10 rounded-xl bg-white scale-y-105 text-lg text-purple-800 shadow-lg shadow-purple-300 hover:scale-125 hover:text-white hover:bg-purple-900 transition-all duration-[.1s]"
